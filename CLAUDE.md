...
- Apartados actuales:
  ...
  3. **Estado de la investigación** — en qué punto está hoy, y si está publicada, dónde leerla. Va aparte porque no describe al estudio ni a quienes lo hacen: es un dato que cambia con el tiempo. Los campos de seguimiento que se sumen (fecha de aprobación, revista) van acá.
...
- **Campos condicionales** (los que solo aplican a una respuesta): el campo vive siempre en el HTML y `static/js/estudio_form.js` lo oculta salvo cuando corresponde. Se declara en el template, sin tocar el JS: `data-depende-de="<id del select>"` + `data-mostrar-si="<texto de la opción>"` (el texto sale de la constante de Python, para que no se desincronicen). **El select del que depende necesita `id`** — el JS lo busca con `getElementById`. Hoy lo usan `fuente_datos_otra` (opción `FUENTE_DATOS_OTRA`), `instituciones_detalle` (opción `OTRAS_INSTITUCIONES_SI`) y `publicacion_url` (opción `ESTADO_PUBLICADO`).

### Dónde está publicada una investigación
- Con el estado en **`ESTADO_PUBLICADO`** ("Publicado") aparece `publicacion_url`, la dirección donde se puede leer el artículo. Como todo campo condicional, solo se persiste si esa opción quedó elegida (lo filtra `estado_desde_form`), así no queda un enlace colgado si el estado vuelve para atrás.
- **`normalizar_url()` (en `utils/estudio.py`) es la única puerta de entrada** y hace dos cosas. Acepta lo que la gente tiene a mano: una URL completa se guarda tal cual, `revista.com/x` se convierte en `https://revista.com/x` y un DOI suelto (`10.1016/…`, con o sin el prefijo `doi:`) se convierte en `https://doi.org/…`. Y **descarta cualquier esquema que no sea http/https**: el valor lo escribe una persona y termina en un `href` que van a tocar los demás, así que un `javascript:` ahí sería XSS. Por eso los templates pueden poner el valor en el `href` sin filtrar de nuevo.
- Se muestra en los tres lados: en la ficha como botón "Leer la publicación" (más la dirección cruda debajo, que es dato del registro), en la tarjeta del repositorio como acción del pie —**no dentro de la tarjeta, que ya es un `<a>` entero y no se pueden anidar**— y en la tabla como enlace en su celda. Las columnas que se renderizan como enlace están en `COLUMNAS_ENLACE` (`utils/repositorio.py`), que las rutas pasan a las dos vistas.
